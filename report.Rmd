...
ggplotly(
  edu %>%
    ...
    labs(
      x = "WBL index (2021)",
      y = "Education attainment index (2021)",
      color = NULL,
      shape = NULL,
      title = "Association between WBL index and Education attainment index"
    ),
  tooltip = "text"
) %>%
  layout(
...
health <- gap %>%
  filter(
    indicator == "Global Gender Gap Health and Survival Subindex",
    subindicator_type == "Index"
  ) %>%
  select(country_name,
         "2021") %>%
...
health$tooltip <-
  glue_data(
    ...
    "Country: {economy}",
    "\nWBL: {wbl_index}",
    "\nEducation Attainment Index: {health_survival}"
  )
```
...
ggplotly(
  health %>%
    ...
    labs(
      x = "WBL index (2021)",
      y = "Health survival index (2021)",
      color = NULL,
      shape = NULL,
      title = "Association between WBL index and health survival index"
    ),
  tooltip = "text"
) %>%
  layout(
